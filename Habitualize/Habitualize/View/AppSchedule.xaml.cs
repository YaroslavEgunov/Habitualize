...
public partial class AppSchedule : ContentView, INotifyPropertyChanged
{
    ...
    private async void UpdateHabitList(DateTime date)
    {
        ...
        var habitsForSelectedDate = _habits.Where(h => 
        h.RepeatSchedule.Date == date.Date).ToList();
        Events.Clear();
        var sortedHabits = _habits.OrderBy(habit => habit.RepeatSchedule);
        var groupedHabits = sortedHabits
            .GroupBy(habit => habit.RepeatSchedule.Date)
            .ToDictionary(group => group.Key, 
            group => group.Select(habit => habit.HabitName)
            .ToList());
